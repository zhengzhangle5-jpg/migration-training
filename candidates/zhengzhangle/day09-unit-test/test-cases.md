...
| 用例ID   | 测试场景        | 输入           | 预期行为 |
|--------|-------------|--------------|------|
| TC-011 | resultset为空 | 空LIST        | 返回空 List |
| TC-012 | SQL执行异常     | SQLException | 向上抛出异常 |

### 2.2 EmployerReportDao（聚合统计）
...
| 用例ID   | 测试场景 | 输入        | 预期行为     |
|--------|------|-----------|----------|
| TC-030 | 查询不存在 ID | year=1999 | 返回 空LIST |

#### 2.32 异常场景测试

| 用例ID   | 测试场景 | 输入           | 预期行为    |
|--------|------|--------------|---------|
| TC-031 | NVL 生效 | year=2023    | BigDecimal.ZERO  |
| TC-032 | SQL 执行异常（聚合） | SQLException | 向上抛出异常  |
